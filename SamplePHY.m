...
crc8 = crc.generator([1,1,0,1,0,0,1,1,1]); %x^8+x^7+x^5+x^2+x+1 0xA7 CRC-8-Bluetooth
...
%     end
    ...
    for j = 0:length(frame_crc)-1
        ...
        % Every bit in frame_crc lasts for 44 in length(carrier).
    end
...
for i = 1:length(RxFIFO)
    ...
    elseif state == 1
        ...
        if length(decodeFIFO) == 44*108
            ...
            % smooth(~, 10) is equivalent to smooth(~, 9)
